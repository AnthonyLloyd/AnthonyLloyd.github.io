...
/// Derivative approximation using two function evaluations at $x \pm h$.    
...
let richardsonStoppingCriteria acc s = Seq.map Seq.last s |> Seq.triplewise |> Seq.find (fun (a,b,c) -> abs(a-b)<=acc && abs(b-c)<=acc) |> trd

let derivativeSeq f x h = Seq.unfoldInf ((*)0.5) h |> Seq.map (derivative2 f x)

/// Derivative accurate to tol using Richardson extrapolation
let derivative acc h f x = derivativeSeq f x h |> richardsonExtrapolationEven |> richardsonStoppingCriteria acc

let integralSeq f a b =
    let h0 = (b-a)*0.5
    let i0 = (f a+f b)*h0            
...
let integral acc f a b = if a=b then 0.0 else integralSeq f a b |> richardsonExtrapolationEven |> richardsonStoppingCriteria acc
